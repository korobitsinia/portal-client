...
import {changeSearchValue, downloadAllPeople, setActiveDepartment} from "../../../../redux/phonebook/actions";
import {arraySearch} from "../../../../helpers/halpersFunctions";
import styles from './SearchPopup.module.scss'

const SearchPopup = ({close}) => {
	const dispatch = useDispatch()
	const {search: {value}, allPeople} = useSelector(({phonebook}) => phonebook)
		...
		<Modal close={close} title={"Поиск"} width={1100}>
			<div className={styles.search}>
				<div className={styles.header}>
					<Input placeholder="Начните печатать для поиска" value={value}
				...
				<div className={styles.block}>
					{arraySearch(allPeople, value).length ?
						...
						<div className={styles.empty}>
							Поиск не дал результатов. <br/> Проверьтие правильность введенного запроса
						</div>
					}
				</div>
			</div>
